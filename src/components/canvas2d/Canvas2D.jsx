...
// 画布宽高比，必须和 mathToScreen 里默认视图范围的比例一致，
// 否则格子会变成长方形、圆会变成椭圆
const 宽高比 = 1.5;
...
function Canvas2D({ 函数列表 }) {
  ...
  const [鼠标位置, 设置鼠标位置] = useState(null); // 数学坐标，离开时为 null
  ...
  useEffect(() => {
    ...
    函数列表.forEach((项) => {
      ...
      try {
        ...
        // 这条函数画不出来就跳过，但要在控制台留痕，否则排查时一片空白
